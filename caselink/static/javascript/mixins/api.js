...
module.exports = {
  methods: {
    ...
    _getFailure: function(data) {
      if(data.id){
        return this._restAjax('GET', `/failure/${data.id}/`);
      } else if (data.failure_regex || data.autocase_pattern) {
        return this._restAjax('GET', `/failure/?failure_regex=${data.failure_regex || ''}&autocase_pattern=${data.autocase_pattern || ''}`)
          .then(res => {
    ...
    _createFailure: function(data) {return this._restAjax('POST', '/failure/', data);},
    _updateFailure: function(data) {return this._restAjax('PUT', `/failure/${data.id}`, data);},
    _deleteFailure: function(data) {return this._restAjax('DELETE', `/failure/${data.id}`);},
